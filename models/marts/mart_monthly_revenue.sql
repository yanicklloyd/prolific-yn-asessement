...
monthly_revenue as (
    select
        ...
        round(sum(case when is_payment = 1 then transaction_amount else 0 end ), 2) as gmv_gbp,
        ...
        max(coalesce(spend_threshold, null)) as spend_threshold,
        max(coalesce(cumulative_payment_gbp, 0)) as cumulative_payment_gbp,
